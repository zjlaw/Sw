...
def get_obs(pipe):
    ...
    if pose:
            ...
            heading   =  m.atan2(2.0 * (w*z + x*y), w*w + x*x - y*y - z*z) * 180.0 / m.pi
...
def get_reward(state_n, act):
    ...
    if abs(omega) < omega_lim:
         omega_reward = abs(omega) * omega_factor
    else:
         omega_reward = neg_reward
    
    if np.where(actions == act)[0][0] != stop_index:
        invert_dif = 1 / (2 * (np.where(actions == act)[0][0] - np.where(actions == stop_index)[0][0]))
        action_reward = invert_dif - action_factor
    else:
        action_reward = 0.5

    reward = vel_reward + heading_reward + omega_reward + action_reward
    return reward, vel_reward, heading_reward, omega_reward, action_reward


    
def log_data(time_now, xvel, heading, omega, act):
    time_now.append(dt.now().strftime("%Y_%m_%d_%H_%M_%S"))
    velocity.append(xvel)
    yaw.append(heading)
    yaw_dot.append(omega)
    action_taken.append(act)
    # data = (velocity, yaw, yaw_dot)
    return time_now, velocity, yaw, yaw_dot, action_taken

def export_data(episode, time_now, velocity, yaw, yaw_dot, action_taken, velocity_reward, heading_reward, omega_reward, action_reward, track_reward):
    ...
    for i, vel in enumerate(velocity):
        ws.write(i+1, 1, vel)
    for i, vel_rew in enumerate(velocity_reward):
        ws.write(i+1, 2, vel_rew)
    for i, head in enumerate(yaw):
        ws.write(i+1, 3, head)
    for i, head_rew in enumerate(heading_reward):
        ws.write(i+1, 4, head_rew)
    for i, ome in enumerate(yaw_dot):
        ws.write(i+1, 5, ome)
...
# print("Would you like to exploite symmetry? y/n ")
# if input() == 'y':
#     sym = True
#     def get_state(xvel,heading, omega):
#         xvel = int(np.digitize(xvel, vel_space)) - 1
#         heading = int(np.digitize(heading, heading_space)) - 1
#         sym_heading = int(np.digitize(-heading, heading_space)) - 1
#         omega = int(np.digitize(omega, omega_space)) - 1
#         sym_omega = int(np.digitize(-omega, omega_space)) - 1
#         return xvel, heading, omega, sym_heading, sym_omega
# else:
#     sym = False
#     def get_state(xvel,heading, omega):
#         xvel = int(np.digitize(xvel, vel_space)) - 1
...
for e in range(episodes):
    ...
    ep_reward = 0
    pipe = rs.pipeline()
    velocity = []; yaw = []; yaw_dot = []; action_taken = []; time_now = []; track_reward = []
    ...
    time_now, velocity, yaw, yaw_dot, action_taken  = log_data(time_now, xvel, heading, omega, act)
    for i in range(iterations):
        ...
        time_now, velocity, yaw, yaw_dot, action_taken = log_data(time_now, xvel_n, heading_n, omega_n, act)
        # if sym == False:
